...
class DatalistReader:
    ...
    def yield_chunks(self):
        ...
        logger.info(f"Datalist {os.path.basename(self.src_fn)} yielded {len(entries)} intersecting files.")

        for entry in entries:
            reader = StreamFactory.get_reader(entry['path'], region=self.region, **self.kwargs)
            if not reader:
                continue
